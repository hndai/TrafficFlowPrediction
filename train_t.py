...
def train_model_bak(model, X_train, y_train, name, config):
    """train
...
def main(argv):
    ...
    X_train_a = pd.read_csv('data/train_a1.csv', header=0)
    #X_train_a = pd.read_csv('data/train.csv', header=0, index_col=0, date_parser=parser,
                 #squeeze=True, parse_dates=[0])
    y_train_a = pd.read_csv('data/test_a1.csv', header=0)
    ...
    if args.model == 'arima':
        #X_train = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 1))
        train, test = X_train_a.values, y_train_a.values
        #train=train[1:]
        #test=test[1:]
        print("train_data:", train)
        history = [x for x in train]
        print("history:", history)
        m = model_t.arima(history,test)
        #train_model(m, X_train, y_train, args.model, config)

    if args.model == 'arima1':
        #X_train = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 1))
        train, test = X_train_a.values, y_train_a.values
        #train=train[1:]
        #test=test[1:]
